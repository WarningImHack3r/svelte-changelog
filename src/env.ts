...
export const variables = defineEnvVars({
	...
	POSTHOG_KEY: {
		...
		schema: v.optional(v.pipe(v.string(), v.regex(USUAL_TOKEN_REGEX)), undefined)
	},
	POSTHOG_ENV_ID: {
	 ...
	 * All of them are mandatory for simplicity sake, enter a dummy value on the 3's
	 * if you just want to use the GITHUB_TOKEN (or the opposite).
	 */
	GH_APP_ID: {
		static: true,
	...
	GH_APP_PRIV_KEY_BASE64: {
		...
		schema: v.pipe(v.string(), v.regex(v.BASE64_REGEX))
	},
	GITHUB_TOKEN: {
		/*
		...
		schema: v.pipe(v.string(), v.regex(USUAL_TOKEN_REGEX))
	},
	// Cloudflare
	CLOUDFLARE_ZONE_ID: {
		static: true,
		description: "Cloudflare's zone ID for caching purposes",
		schema: v.optional(v.pipe(v.string(), v.regex(DASHLESS_UUID_LIKE_REGEX)), undefined)
	},
	...
	REDIS_URL: {
		...
		schema: v.optional(v.pipe(v.string(), v.url()), undefined)
	},
	...
	WEBHOOKS_REPLICATOR_TOKEN: {
		...
		schema: v.optional(v.pipe(v.string(), v.regex(v.BASE64_REGEX)), undefined)
	},
